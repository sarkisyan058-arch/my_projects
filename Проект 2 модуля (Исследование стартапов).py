...
# __Комментарий от ревьюера №1__
# 
# Для чего заново выгружаешь пандас?
...
# __Комментарий от ревьюера №1__
# 
# На этом этапе всё хорошо. Ты корректно выгрузил данные и посмотрел их содержимое.  Можно приступать к следующему шагу
# </div>
...
# __Комментарий от ревьюера №1__
# 
# Артём, тут правильнее дополнительно построить гистограмму распределение с выделением массовых и нишевых сегментов. Примерно так ![image.png](attachment:image.png)
#     
# <div class="alert alert-info"> <b>Добавил гистограмму распределения сегментов с цветовой разметкой:
# 
# зелёный — массовые сегменты (≥ mass_threshold);
# 
# оранжевый — средние сегменты (между mid_threshold и mass_threshold);
# 
# красный — нишевые сегменты (< mid_threshold).
#                                              
# Горизонтальные линии-пороги на графике:
# 
# пунктир зелёного цвета — порог массовых сегментов;
# 
# пунктир оранжевого цвета — порог средних сегментов.
# 
# Подпись и оформление:
# 
# заголовок с пояснением цветовой схемы;
# 
# подписи осей;
# 
# сетка для удобства чтения;
# 
# легенда с указанием значений порогов.</b> Исправленно.</div>
...
# __Комментарий от ревьюера №1__
# 
# Верно, так как в датасете есть данные о финансировании за последний день года и о компаниях, основанных в последний месяц года, считаем, что данные за 2014 год полные.
...
# 
#     
# Мы берём годы начиная с года когда выполняются условия > 50
#     
#     
# <div class="alert alert-info"> <b>добавлено и изменено: Фильтрация данных перед графиком:
# 
# filtered_annual_stats = annual_stats[annual_stats['total_rounds_year'] > 50] — создаём отфильтрованную версию датафрейма: берём только строки, где количество раундов за год превышает 50.
# 
# Проверка на пустоту:
# 
...
#     
#     
# <div class="alert alert-info"> <b>Тут так же поправил условия задания</b> Исправленно.</div>
...
#     
#     
# <div class="alert alert-info"> <b>тут тоже поправил , что бы код соответствовал услоывиям задачи, так же пришлось скорректировать вывод :</b> Исправленно.</div>
...
# 
#     
# Тут аналогично, данные берем с 1999
#     
#     
# <div class="alert alert-info"> <b>поправил код с условиями: интересно, что все значения return_ratio_clean = 1.0;</b> Исправленно.</div>
...
# 
#     
# Кажется график не совсем корректный. У тебя везде среднее по каждого типу 1. Он должен получится примерно таким ![image.png](attachment:image.png)
#     
# <div class="alert alert-info"> <b>переделал понкт, надеюсь верно)</b> Исправленно.</div>
...
# 
#     
# Давай поправим выводы после корректировки
#     
#     
# <div class="alert alert-info"> <b>Комментарии студента:</b> Исправленно.</div>

# Привет! хотел бы дополнить ещё один момент по поводу проделайнной работы))
# 
# где-то пришлось импортировать библиотеку повторно, потому что возникала ошибка,
# 
# где-то приходилось разбивать код на блоки, даже если это не логично, это связанно с проблемой с моим ПК, 
# 
# скорее всего перегрев. Надеюсь моя работа понравится))
...
# __Комментарий от ревьюера №1__
# 
# Работа хорошая. А какие ошибки возникают, если повторно не выгрузить библиотеку?
#     
# - Возникает ошибка, что библиотека не загруженна, хотя я делал рестарт , запускал все по очередно, но почему то все равно выскакивает ошибка
